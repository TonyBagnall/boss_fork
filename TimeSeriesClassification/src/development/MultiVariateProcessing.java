...
public class MultiVariateProcessing {
    ...
    public static void makeConcatenatedFiles(){
        ...
        String[] probs={"SelfRegulationSCP1","SelfRegulationSCP2"};
        //probs=DataSets.mtscProblems2018;
    ...
    public static void makePCAFiles() throws Exception{
        String path="Z:\\Data\\ConcatenatedMTSC\\";
        String dest="Z:\\Data\\ConcatenatedMTSC_PCA\\";
        OutFile out=new OutFile(path+"SummaryData.csv");
        out.writeLine("problem,numTrainCases,numTestCases,numDimensions,seriesLength,numClasses");
        ...
        for(String prob:DataSets.mtscProblems2018){
            ...
            if(!(t1.exists()||t2.exists())){
                ...
                System.out.println("Num train instances ="+train.numInstances());
                System.out.println("Num test instances ="+test.numInstances());
                System.out.println("num train attributes ="+train.numAttributes());
                System.out.println("num classes="+train.numClasses());
                PrincipalComponents filter = new PrincipalComponents();
                //filter.setNormalize(false);
                filter.setVarianceCovered(0.95);
                filter.setInputFormat(train); 
                Instances pcaT = Filter.useFilter(train, filter);
    //Build a new train test file of concatenated attributes
                File f= new File(dest+prob);
                f.mkdirs();
                OutFile pcaTrain=new OutFile(dest+prob+"\\"+prob+"_TRAIN.arff");
                OutFile uniTest=new OutFile(dest+prob+"\\"+prob+"_TEST.arff");;
                pcaTrain.writeString(pcaT+"");
                
    //            System.out.println(" Object type ="+x);
                train = ClassifierTools.loadData(dest+prob+"\\"+prob+"_TRAIN");
                System.out.println("Can load univariate "+dest+prob+"\\"+prob+"_TRAIN");
                System.out.println("Num atts ="+(train.numAttributes()-1)+" num train cases = "+train.numInstances());
                test = ClassifierTools.loadData(dest+prob+"\\"+prob+"_TEST");
                System.out.println("Can load univariate "+dest+prob+"\\"+prob+"_TEST");
...
//        Instances mtsc=MultivariateInstanceTools.convertUnivariateToMultivariate(temp,30);
    ...
    public static void makeSingleDimensionFiles(){
         String path="Z:\\Data\\MultivariateTSCProblems\\";
        for(String prob: DataSets.mtscProblems2018){
            if(prob.equals("InsectWingbeat")||prob.equals("DuckDuckGeese")||prob.equals("PEMS-SF")||prob.equals("FaceDetection"))
                continue;
...
//ID
        int id=129;
        OutFile out=new OutFile(path+"SummaryData.csv");
        out.writeLine("ID,Dataset,Donator1,Donator2,Train_size,Test_size,Length,Number_of_classes,Type,Best_algorithm,Best_acc,Original_source,Paper_first_used,Image,Description,First_link,Second_link,First_used_TSC,TimeStamp,Multivariate,Dimensions"); 
        ...
        for(String prob: DataSets.mtscProblems2018){
            ...
            Instance temp=train.instance(0);
            Instances x= temp.relationalValue(0);
            int numDimensions=x.numInstances();
            System.out.println(" number of dimensions "+x.numInstances());
            int length=x.numAttributes();
            System.out.println(" number of attributes per dimension "+x.numAttributes());
            out.writeLine((id++)+","+prob+",,,"+train.numInstances()+","+test.numInstances()+","+length+","+train.numClasses()+",,,,,,,,,,,,1,"+numDimensions);
...
//1. Format into a standard flat ARFF, then make into a multivariate problem.  BCI II data set ia  
  public static void formatSelfRegulationSCP1() throws Exception {
    ...
    String path="E:\\Data\\BCI Competition 2\\Data Set 1a\\";
...
//Labels in a separate text file
    ...
    public static void makeFileForPlots(){
        
        for(String prob: DataSets.mtscProblems2018){
            if(prob.equals("Heartbeat")){
                OutFile out = new OutFile("Z:\\Data\\MultivariateTSCProblems\\Plots\\"+prob+".csv");
                Instances train =ClassifierTools.loadData("Z:\\Data\\MultivariateTSCProblems\\"+prob+"\\"+prob+"_TRAIN");
                Instance first=train.instance(0);
                Instances x=first.relationalValue(0);
                for(int i=0;i<x.instance(0).numAttributes();i++){
                    for(Instance ins:x){
                        out.writeString(ins.value(i)+",");
                    }
                    out.writeString("\n");
                }
            }
    ...
    public static void makeAllZips() throws IOException{
        for(String prob: DataSets.mtscProblems2018){
           DataSets.pack("E:\\Multivariate\\MultivariateTSCProblems\\"+prob,"E:\\Multivariate\\Zips\\"+prob+".zip");
        }
    }
    
    public static void main(String[] args) throws Exception {
//        ="Z:\\Data\\MultivariateTSCProblems\\";
        String p="E:\\Multivariate\\MultivariateTSCProblems\\";
