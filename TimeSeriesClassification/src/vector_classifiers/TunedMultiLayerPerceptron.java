...
1. number of hidden layers: 2 values
...
3. Learning rate: 4 values, data independent, variable m_learningRate
1,1/4,1/8,1/32,1/128
4. Momentum: 4 values
0,1/4,1/2,3/4
5. m_decay 2 values: either true or false
6. number Epochs 250,500,1000,1500

total models = 2*4*4*4*2*4=1024
...
public class TunedMultiLayerPerceptron extends MultilayerPerceptron implements SaveParameterInfo,TrainAccuracyEstimate,SaveEachParameter,ParameterSplittable{
    ...
    protected int[] paraSpace5;//Number of epochs
...
//Para 1: structure: 8 settings
        ...
        this.setHiddenLayers(paraSpace1[para1Value]);
//Para 2: Learning rate: 4 settings
          int remainder=x%128;
        int para2Value=remainder/32;
        this.setLearningRate(paraSpace2[para2Value]);
//Para 3: Momentum: 4 settings
        int para3Value=(x%32)/8;
        this.setMomentum(paraSpace3[para3Value]);
//Para 4: Decay: 2 settings
        int para4Value=(x%8)/4;
        if(para4Value==0)
            this.setDecay(false);
        else
            this.setDecay(true);
//Para 5: Epochs: 4 settings
        int para5Value=x%4;
        this.setTrainingTime(paraSpace5[para5Value]);
        System.out.println(para1Value+","+para2Value+","+para3Value+","+para4Value+","+para5Value);
     }

    public String deleteWhenFinshed(int x) {
        x=x-1;
//Para 1: structure: 8 settings
        int para1Value=x/128; // First block para 1
        this.setHiddenLayers(paraSpace1[para1Value]);
...
//        int paraSpaceSize=8*4*4*2*4=1024;


        System.out.println(para1Value+","+para2Value+","+para3Value+","+para4Value+","+para5Value);
        return para1Value+","+para2Value+","+para3Value+","+para4Value+","+para5Value;
...
//TO DO: Write the other stats        
    ...
    public final void setStandardParaSearchSpace(int m){
        paraSpace1=new String[8]; //# hidden layers imbedded in the structure
...
/*        paraSpace1[8]="a,i";
        ...
        paraSpace1[18]="t,i";
        paraSpace1[19]="t,o";
*/        paraSpace2=new double[]{1.0,1/4.0,1/32.0,1/128.0};//Learning rate
        paraSpace3=new double[]{0,1/4.0,1/2.0,3/4.0};//momentum
        paraSpace4=new boolean[2];
        paraSpace4[0]=true;
        paraSpace4[1]=false;
        paraSpace5=new int[]{250,500,1000,1500};
        System.out.println("Number of parameters for each ="+paraSpace1.length*paraSpace2.length*paraSpace3.length*paraSpace4.length*paraSpace5.length);
    ...
    public void tuneMLP(Instances train) throws Exception {
            ...
            for(double p2:paraSpace2){//learning rate
                for(double p3:paraSpace3){//Num trees
                    for(boolean p4:paraSpace4){//Decay
                        for(int p5:paraSpace5){//num epochs
                            count++;
        ...
        //                                System.out.println("SKIPPING THIS FOLD");
                            ...
                            model.setDecay(p4);
                            model.setTrainingTime(p5);
                            tempResults=cv.crossValidateWithStats(model,trainCopy);
                            tempResults.setName("MLPPara"+count);
                            tempResults.setParas("HiddenNodes,"+p1+",LearningRate,"+p2+",Momentum,"+p3+",Decay,"+p4+",numEpochs,"+p5);
                            double e=1-tempResults.acc;
                            if(debug)
                                System.out.println("HiddenNodes,"+p1+",LearningRate,"+p2+",Momentum,"+p3+",Decay,"+p4+",numEpochs,"+p5+", Acc = "+(1-e));
                            ...
                            if(saveEachParaAcc){// Save to file and close
                                ...
                                if(e<minErr){
                                    ...
                                    ties.add(new ResultsHolder(p1,p2,p3,p4,p5,tempResults));
                                }
...
/* SET UP FINAL MODEL */
                ...
                this.setTrainingTime(bestEpochs);
                res=best.res;
...
//        jamesltests();
        ...
        t.debug=true;
        t.setStandardParaSearchSpace(100);
        HashSet<String> hs= new HashSet<>();
        for(int i=1;i<=1024;i++){
            String s=t.deleteWhenFinshed(i);
            if(hs.contains(s))
                System.out.println("ERROR on key :"+i+" Duplicate combination "+s);
            hs.add(s);
        }
        System.exit(0);
    ...
    public static class ResultsHolder{
        ...
        ResultsHolder(String a, double b,double c, boolean d, int e,ClassifierResults r){
            nodes=a;
            lRate=b;
            mRate=0;
            decay=d;
